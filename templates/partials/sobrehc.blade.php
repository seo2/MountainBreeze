
<div class="pt-16 -mt-12 bg-top bg-no-repeat lg:bg-contain" style="background-image: url('<?php bloginfo('template_url') ?>/dist/img/bg_naranjo.png');">
    <div class="container mx-auto">
        <div class="grid grid-cols-12 gap-4 lg:gap-12">
            <div class="col-start-1 col-span-12 lg:col-start-2 lg:col-span-5 mb-24 lg:mb-12 -mt-24 relative z-1">
                <img src="<?php bloginfo('template_url') ?>/dist/img/fotorara.png" alt="Herencia Colectiva" class="-ml-12 lg:ml-auto">
            </div>
            <div class="bg-naranjo col-start-1 col-span-12 pt-12 px-4 -mt-32 lg:mt-0  lg:col-span-5 mb-8">
                <div class="relative mt-3">
                    <p class="text-beige text-lg lg:text-xl uppercase">Herencia Colectiva</p>
                    <h4 class="text-beige text-4xl lg:text-4xl font-bold leading-none my-3 font-festivo6 w-9/12 lg:w-full">Transmisión y creación de un legado común</h4>
                    <p class="text-negro mb-4">Queremos facilitar saberes, aprendizajes y prácticas que posibiliten formas más colaborativas y conscientes de vivir, experimentar y crear.</p>
                    <a href="#" class="h-12 w-full lg:w-9/12 leading-12 uppercase text-center inline-block border border-negro border-solid text-negro hover:bg-negro hover:border-negro hover:text-beige transition duration-200">
                        Conocer más sobre Herencia Colectiva
                    </a>
                </div>
            </div>
        </div>
    </div>

</div>
<div class="pt-16 -mt-48 h-48 w-full" style="background: url('<?php bloginfo('template_url') ?>/dist/img/bg_naranjo_full.png') bottom center no-repeat; background-size: 100% auto;">


</div>