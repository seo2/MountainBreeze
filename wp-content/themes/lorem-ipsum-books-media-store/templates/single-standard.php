...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_single_standard_output' ) ) {
	function lorem_ipsum_books_media_store_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && lorem_ipsum_books_media_store_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
		<?php 
		}

		if (!$post_data['post_protected'] && (
			!empty($post_options['dedicated']) ||
			(lorem_ipsum_books_media_store_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])			)) {
			?>
		...
		if ($post_data['post_protected']) {
			...
			if (function_exists('lorem_ipsum_books_media_store_reviews_wrapper')) {
				...
				lorem_ipsum_books_media_store_show_layout(lorem_ipsum_books_media_store_gap_wrapper($post_data['post_content']));
			}
			wp_link_pages( array( 
		...
		lorem_ipsum_books_media_store_close_wrapper();				
		if (!$post_data['post_protected']) {
		...
		if (!$sidebar_present) lorem_ipsum_books_media_store_close_wrapper();			get_template_part(lorem_ipsum_books_media_store_get_file_slug('templates/_parts/related-posts.php'));
		if ($sidebar_present) lorem_ipsum_books_media_store_close_wrapper();		
		// Show comments
