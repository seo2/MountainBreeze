...
if ( !function_exists( 'lorem_ipsum_books_media_store_core_customizer_save_options' ) ) {
		...
		parse_str(lorem_ipsum_books_media_store_get_value_gp('data'), $data);
		...
		if ($mode == 'save') {
			if (is_array($colors) && count($colors) > 0) {
				...
				foreach ($order as $slug) {
					...
					if (empty($new_slug)) $new_slug = lorem_ipsum_books_media_store_get_slug($schemes['title']);
...
if ( !function_exists( 'lorem_ipsum_books_media_store_core_customizer_scheme_delete' ) ) {
		...
		$colors = (array) lorem_ipsum_books_media_store_storage_get('custom_colors');
		$order = !empty($_POST['order']) ? explode(',', lorem_ipsum_books_media_store_get_value_gp('order')) : array_keys($colors);
...
if ( !function_exists( 'lorem_ipsum_books_media_store_core_customizer_scheme_copy' ) ) {
		...
		$scheme = lorem_ipsum_books_media_store_get_value_gp('scheme');
		$colors = lorem_ipsum_books_media_store_storage_get('custom_colors');
		$order = !empty($_POST['order']) ? explode(',', lorem_ipsum_books_media_store_get_value_gp('order')) : array_keys($colors);
		$response = array( 'error' => '' );
