...
if ( !function_exists( 'lorem_ipsum_books_media_store_show_logo' ) ) {
	function lorem_ipsum_books_media_store_show_logo($logo_main=true, $logo_fixed=false, $logo_footer=false, $logo_side=false, $logo_text=true, $logo_slogan=true) {
		...
		if (empty($logo_main) && empty($logo_fixed) && empty($logo_footer) && empty($logo_side) && empty($logo_slogan))
		...
		<div class="logo">
			<a href="<?php echo esc_url(home_url('/')); ?>"><?php
				if (!empty($logo_main)) {
					...
					echo '<img src="'.esc_url($logo_main).'" class="logo_main" alt="' . esc_attr__('Image', 'lorem-ipsum-books-media-store') . '"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
				if (!empty($logo_fixed)) {
					$attr = lorem_ipsum_books_media_store_getimagesize($logo_fixed);
					echo '<img src="'.esc_url($logo_fixed).'" class="logo_fixed" alt="' . esc_attr__('Image', 'lorem-ipsum-books-media-store') . '"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
				if (!empty($logo_footer)) {
					$attr = lorem_ipsum_books_media_store_getimagesize($logo_footer);
					echo '<img src="'.esc_url($logo_footer).'" class="logo_footer" alt="' . esc_attr__('Image', 'lorem-ipsum-books-media-store') . '"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
				if (!empty($logo_side)) {
					$attr = lorem_ipsum_books_media_store_getimagesize($logo_side);
					echo '<img src="'.esc_url($logo_side).'" class="logo_side" alt="' . esc_attr__('Image', 'lorem-ipsum-books-media-store') . '"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
	...
	function lorem_ipsum_books_media_store_core_frontend_scripts() {
		...
		if (is_array($theme_fonts) && count($theme_fonts) > 0) {
			...
			if ($google_fonts) {
				...
				*/
				$google_fonts_enabled = ( 'off' !== esc_html_x( 'on', 'Google fonts: on or off', 'lorem-ipsum-books-media-store' ) );
				if ( $google_fonts_enabled ) {
					wp_enqueue_style( 'lorem-ipsum-books-media-store-font-google-fonts-style', add_query_arg( 'family', $google_fonts . '&subset=' . lorem_ipsum_books_media_store_get_theme_option('fonts_subset'), "//fonts.googleapis.com/css" ), array(), null );
		...
		wp_enqueue_style( 'fontello-style',  lorem_ipsum_books_media_store_get_file_url('css/fontello/css/fontello.css'),  array(), null);
		...
		if (in_array(lorem_ipsum_books_media_store_get_theme_option('menu_hover'), array('slide_line', 'slide_box'))) {
			wp_enqueue_script( 'slidemenu-script', lorem_ipsum_books_media_store_get_file_url('js/jquery.slidemenu.js'), array('jquery'), null, true );
		}
		...
		if (lorem_ipsum_books_media_store_get_custom_option('show_video_bg') == 'yes' && lorem_ipsum_books_media_store_get_custom_option('video_bg_youtube_code') != '') {
			wp_enqueue_script( 'tubular', lorem_ipsum_books_media_store_get_file_url('js/jquery.tubular.1.0.js'), array('jquery'), null, true );
		}
...
if ( !function_exists( 'lorem_ipsum_books_media_store_enqueue_slider' ) ) {
	function lorem_ipsum_books_media_store_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			wp_enqueue_style(  'swiperslider-style', 			lorem_ipsum_books_media_store_get_file_url('js/swiper/swiper.css'), array(), null );
			// jQuery version of Swiper conflict with Revolution Slider!!! Use DOM version
			wp_enqueue_script( 'swiperslider-script', 			lorem_ipsum_books_media_store_get_file_url('js/swiper/swiper.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'lorem_ipsum_books_media_store_enqueue_polaroid' ) ) {
	function lorem_ipsum_books_media_store_enqueue_polaroid() {
		wp_enqueue_style(  'polaroid-style', 	lorem_ipsum_books_media_store_get_file_url('js/photostack/component.css'), array(), null );
		wp_enqueue_script( 'classie-script',		lorem_ipsum_books_media_store_get_file_url('js/photostack/classie.js'), array(), null, true );
		wp_enqueue_script( 'polaroid-script',	lorem_ipsum_books_media_store_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'lorem_ipsum_books_media_store_enqueue_diagram' ) ) {
	function lorem_ipsum_books_media_store_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') wp_enqueue_script( 'diagram-chart-script',	lorem_ipsum_books_media_store_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') wp_enqueue_script( 'diagram-raphael-script',	lorem_ipsum_books_media_store_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if ( !function_exists( 'lorem_ipsum_books_media_store_enqueue_popup' ) ) {
	function lorem_ipsum_books_media_store_enqueue_popup($engine='') {
		if ($engine=='pretty' || (empty($engine) && lorem_ipsum_books_media_store_get_theme_option('popup_engine')=='pretty')) {
			wp_enqueue_style(  'prettyphoto-style',	lorem_ipsum_books_media_store_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			wp_enqueue_script( 'prettyphoto-script',	lorem_ipsum_books_media_store_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else if ($engine=='magnific' || (empty($engine) && lorem_ipsum_books_media_store_get_theme_option('popup_engine')=='magnific')) {
			wp_enqueue_style(  'magnific-popup-style',	lorem_ipsum_books_media_store_get_file_url('js/magnific/magnific-popup.css'), array(), null );
			wp_enqueue_script( 'magnific-script',lorem_ipsum_books_media_store_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
...
if ( !function_exists( 'lorem_ipsum_books_media_store_callback_frontend_editor_save' ) ) {
	function lorem_ipsum_books_media_store_callback_frontend_editor_save() {
		...
		parse_str(lorem_ipsum_books_media_store_get_value_gp('data'), $output);
