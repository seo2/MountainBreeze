			...
			if (type=='text' || type=='textarea' || type=='hidden' || type=='spinner') {
				...
				if (val[i]!='') {
					...
					if (file!='') {
						...
						fld.eq(cnt).after('<a class="lorem_ipsum_books_media_store_options_image_preview" data-rel="popup" target="_blank" href="'+val[i]+'">'+('jpg,png,gif'.indexOf(ext)>=0 ? '<img src="'+val[i]+'"/>' : '<span>'+fname+'</span>')+'</a>');
					}
				}
			} else if (type=='range') {
				fld.eq(cnt).val(val[i]).trigger('change');
