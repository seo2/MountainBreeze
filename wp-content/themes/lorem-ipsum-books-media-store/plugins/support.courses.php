...
if ( !function_exists( 'lorem_ipsum_books_media_store_courses_get_period_links' ) ) {
	function lorem_ipsum_books_media_store_courses_get_period_links($links, $page, $delimiter='') {
		...
		if (in_array($page, array('archives_day', 'archives_month')) && is_object($post) && get_post_type()=='courses') {
			...
			$links = '<a class="breadcrumbs_item cat_parent" href="' . esc_url(get_year_link( $year )) . '">' . wp_kses_post($year) . '</a>';
			if ($page == 'archives_day')
				$links .= (!empty($links) ? $delimiter : '') . '<a class="breadcrumbs_item cat_parent" href="' . esc_url(get_month_link( $year, $month )) . '">' . trim(lorem_ipsum_books_media_store_get_date_translations(date('F', $dt))) . '</a>';
		}
		return $links;
	}
}
