...
function wpshout_frontend_post() {
    ...
    ?>
        ...
        <form class="space-y-6" action="#" method="POST" id="new_post" name="new_post" >   
            <input type="hidden" name="remember" value="true">
            <div class=" -space-y-px">
                <div class="relative">
                    <label for="tit_proyecto" class="sr-only">Título del proyecto</label>
                    <input id="tit_proyecto" name="title" type="text" autocomplete="tit_proyecto" required class="appearance-none rounded-none mb-3 relative block w-full px-3 py-4 border border-gray-300 placeholder-gray-500 text-negro focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Título del proyecto">
                </div>
                <div class="relative">
                    <label for="portada" class="sr-only">Sube la portada</label>
                    <input id="portada" name="portada" type="file" autocomplete="portada" required class="appearance-none rounded-none mb-3 relative block w-full px-3 py-4 border border-gray-300 placeholder-gray-500 text-negro focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Sube la portada">
                </div>
                <div class="relative">
                    <label for="descripcion" class="sr-only">Escribe sobre el proyecto</label>
                    <textarea id="descripcion" name="content" autocomplete="descripcion" required class="appearance-none rounded-none mb-3 relative block w-full px-3 py-4 border border-gray-300 placeholder-gray-500 text-negro focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Escribe sobre el proyecto"></textarea>

                    
                </div>
                <div class="relative">
                    <label for="galeria" class="sr-only">Subir galería de imágenes</label>
                    <input id="galeria" name="galeria" type="file" autocomplete="galeria" required class="appearance-none rounded-none mb-3 relative block w-full px-3 py-4 border border-gray-300 placeholder-gray-500 text-negro focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Subir galería de imágenes">
                </div>
            </div>
            ...
            <div>
              <button type="submit" name="submit" id="submit" value="Publish" class="group relative w-full flex justify-center py-2 px-4 border border-negro text-negro uppercase bg-beige hover:bg-naranjo hover:border-naranjo focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-naranjo">
                Subir Proyecto
              </button>
            </div>
...
function wpshout_save_post_if_submitted() {
    ...
    $post = array(
        ...
        'post_type' 	=> 'proyectos' // Could be: `page` or your CPT
    );
    wp_insert_post($post);
